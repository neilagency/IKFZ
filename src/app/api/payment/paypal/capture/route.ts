 ...
 * =======================
 * GET /api/payment/paypal/capture/?token={paypalOrderId}&PayerID={payerId}
 *
  ...
  try {
    ...
    if (captureResult.status === 'COMPLETED') {
      ...
      await prisma.invoice.updateMany({
        ...
        data: { status: 'paid', paidAt: new Date(), transactionId: captureResult.captureId || '', paymentMethod: 'paypal' },
      });
      ...
      await prisma.orderNote.create({
        data: {
          ...
          note: `PayPal-Zahlung erfolgreich. Capture ID: ${captureResult.captureId}`,
          author: 'system',
        },
      });
